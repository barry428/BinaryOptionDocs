...
2. **处理策略**：
   - **转账重试**：尝试完成之前未成功的BTSE转入
   - **成功处理**：转账成功后，增加frozen_balance，更新为ACTIVE
   - **失败处理**：转账失败或超时，更新为CANCELLED
   - **资源释放**：CANCELLED订单不占用任何资源

3. **补偿示例代码**：
```java
public CompensationResult compensatePendingOrders() {
    // 查询超时的PENDING订单
    List<Order> pendingOrders = orderMapper.findPendingOrdersOlderThan(5, TimeUnit.SECONDS);
    ...
    for (Order order : pendingOrders) {
        try {
            // 尝试BTSE转入
            String transferId = btseTransferService.transferFromBtse(
                order.getUserId(), "REAL", order.getAmount(), 
                order.getId(), "PENDING订单补偿");
